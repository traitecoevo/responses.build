...
- **New `provenance` table.** Where the plant material came from, as data. It was split two ways and encoded differently in each dataset that had it: one as a treatment context plus two quantities filed under treatments, the other as an entity context with coordinates in prose. A provenance is a property of the seed origin, not of the growing environment, which is why those quantities are no longer growth conditions. The context it hangs off may be of any category — whether a provenance is a treatment depends on whether the study chose provenances deliberately, which is a fact about the experiment. New: `get_provenance_properties()`.
- **`treatments` becomes `growth_conditions`**, and records a condition whether or not the study manipulated it. The old table held only what an experiment *changed*, so a study that grew everything at 26 °C recorded nothing while one that grew plants at 26 and 32 recorded both — leaving the constant-condition studies, the majority, saying nothing about the conditions their plants grew in. An entry naming a treatment level carries its `treatment_context_id` and `manipulated = TRUE`; an entry naming none applies to the whole dataset. `get_treatment_factors()` becomes `get_growth_conditions()`, and `config/treatment_factors.yml` becomes `config/growth_conditions.yml`.
...
- **`build_setup_pipeline()` writes a `targets` pipeline, and that is the default.** It branches over the datasets rather than naming each as its own targets, so adding a dataset needs no regeneration, and it rebuilds only what changed — touching one AusFizz dataset invalidates 7 targets of 174 where the previous pipeline rebuilt all 31 datasets. Compilation config is declared as file targets, so editing the schema or the definitions invalidates every dataset that read them. Verified identical to the previous build, table for table.
- **The `remake` and `furrr` methods are removed**, along with their templates and the `remake` Remote. `method = "base"` stays as a dependency-free linear script for debugging; the test suite asserts it agrees with the targets pipeline.
...
- **The `traits` table is now `measurements`, and its `trait_name` column is `variable`.** It holds readings -- an instrument reading or measured state at one point of one response -- which is precisely what this fork exists to stop calling traits. `methods` follows suit. There are no derived parameters in this compilation and no `traits` table; fitting curves is out of scope.
- **`curves` is now `responses`, and `curve_id` is `response_id`.** A response is one entity, one time, one set of method settings; a *curve* is a response measured across a driver with more than one point. 88% of AusFizz's responses have exactly one point, so calling them curves needed a disclaimer in three places. `curve` survives where it means an actual curve -- `check_curve_pairing()`, the report's curve plots.
- **`curve_points` is removed.** It stored the wide view alongside the long one, which duplicated 39 Mb of readings and could not carry a reading's `value_type` or `replicates` anyway. `response_pivot_wider()` builds the view on demand -- optionally for one `data_type`, with numeric coercion and empty columns dropped, which is what every caller did by hand.
- The `measurements:` block in `metadata.yml` is now `protocols:` -- one entry per way of measuring, which is what it always described.
- Location properties are `latitude_deg` and `longitude_deg`, not `latitude (deg)`. No identifier in the data model now contains a space or a bracket.
- **A time-valued context no longer disappears from the build.** Where a context lets its values be derived from the data and that column reads as `hms`, `process_format_contexts()` filled `find` from `value` with `ifelse()`, which drops the class of its arguments — so `find` became the underlying seconds (`41400`) while `value` stayed `11:30:00`. `process_create_context_ids()` reads the data column with `as.character()` and looks the result up by `find`, so nothing matched and the context was dropped in silence: `AusFizz`'s `Mitchell_2009` lost all 16 of its `time_of_day_approx` values, and its `temporal_context_id` collapsed to the four replicate levels. The conversion to character now happens before the fill, not after. This is the same defect `metadata_add_contexts()` was fixed for upstream in #49, one layer further down.
- **`individual_id` now identifies an individual.** It was numbered *within* each `taxon_name` and `population_id`, so the same label recurred across taxa and populations: on AusFizz, **22,255 entities carried only 3,844 labels**, and 33 of 35 datasets were ambiguous, so grouping on `individual_id` for a per-plant summary silently merged entities. It is now one number per individual per dataset, and the labels are assigned by sorting rather than by first appearance — the same order-independence already fixed for the context ids.

  The key is numbered, not the label. Where a dataset restarts its own numbering per treatment — `Rep` 1 to 6 in each — numbering the label globally would have merged plants the old scoping kept apart, which is the opposite of the fix.

  **This relabels ids.** Verified across AusFizz's 35 datasets that the entity partition is unchanged (22,255 before and after), that `observation_id` and `response_id` keep identical group counts and partition shapes, that old and new ids are a bijection in both directions, and that no value moved. But 98.7% of `individual_id`, 56.6% of `observation_id` and 53.4% of `response_id` labels differ from what a previous build gave them, so a saved analysis that joins on them needs rebuilding.
- **New `identifier_type`: `organismID`.** Darwin Core's term for an organism tracked through time, which is what a tagged tree at a long-term site needs. `individual_id` is generated per dataset and cannot carry identity across datasets, so a plant measured by several studies — or by one study split into several datasets, as `Drake_2015_a`/`_b`/`_c` is — has no other way to be recognised. It must be declared by a curator: equal individual labels in two datasets are usually coincidence, as AusFizz shows, where four datasets share `Curve_Id` values that are bare integers and not even plants.
- **A column claimed by both the `dataset:` block and `identifiers:` no longer loses its identifiers.** `individual_id: code` together with `var_in: code` is the normal shape for a tagged plant — the tag is the individual within the dataset and the organism across datasets — but `select()` moves a column and the named entry wins, so identifiers lost theirs, `process_add_all_columns()` recreated it as all-NA, and the table came back **empty with no error**. That is the silent loss #232 closed for a `var_in` naming no column, reached instead by one that is consumed. Doubly-claimed columns are now copied back after the rename, and a declared identifier whose column arrives empty by any other route is an error rather than an empty table.
- `metadata_add_identifiers()` reads the allowed identifier types from the schema instead of restating them. The list was hardcoded there as well as declared in `identifier_type`, so the prompt could offer a type that `dataset_test()` would then reject.
