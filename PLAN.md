...
- `ontology/` is **removed** (Stage 7). It was upstream's: 134 terms for a 13-table model plus 11 MB of generated release output for traits.build 1.0.0 and 1.1.0, served from `w3id.org/traits.build`, which points at traits.build's own copy rather than this repo. This package builds 17 tables, calls the per-point readings `variables`, and means derived parameters by `traits` — so the terms described a database it does not build, and it could not have published them anyway. Git keeps the history. **Open:** a `responses.build` ontology covering `curve`, `curve_point`, `driver`, `variable` and `treatment_factor`, importing traits.build's terms where they still apply, with its own w3id registration. Worth doing once curve fitting settles the derived-trait terms — not before, or it needs a second release immediately.
